...
inline int PopCnt64(uint64_t ddw) {
    // 将64位拆分为低32位和高32位
    uint32_t low = (uint32_t)ddw;          // 低32位
    uint32_t high = (uint32_t)(ddw >> 32); // 高32位
    
    // 分别计算低32位和高32位的1的个数，然后相加
    return PopCnt32(low) + PopCnt32(high);
}
