...
int PositionStruct::GenCapMoves(MoveStruct *lpmvs) const {
  ...
  if (sqSrc != 0) {
    ...
    while (sqDst != 0) {
      ...
      int temp_pcCaptured = CHECK_DARK_PIECE(pcCaptured)?pcCaptured-32:pcCaptured;
      if ((temp_pcCaptured & nOppSideTag) != 0) {
  ...
  for (i = ADVISOR_FROM; i <= ADVISOR_TO; i ++) {
    ...
    #ifdef JIEQIMODE
       sqSrc = ucsqPieces[nSideTag + i + 32];
      if (sqSrc != 0) {
        __ASSERT_SQUARE(sqSrc);
        lpucsqDst = PreGen.ucsqAdvisorMoves[sqSrc];
        sqDst = *lpucsqDst;
            ...
            for (int dy = -1; dy <= 1; dy += 2) {
                ...
                if (IN_BOARD(sqDst) && ADVISOR_SPAN(sqSrc, sqDst)) {
                    ...
                    if ((pcCaptured & nSideTag) != 0) {
                        pcCaptured -= nSideTag;
                        // 士追杀车/马/炮/兵
                        if ((pcCaptured >= KNIGHT_FROM && pcCaptured <= ROOK_TO) ||
                            (pcCaptured >= CANNON_FROM && !Protected(sdPlayer, sqDst)) ||
                            (pcCaptured >= PAWN_FROM && AWAY_HALF(sqDst, sdPlayer) && 
                             !Protected(sdPlayer, sqDst))) {
                            return pcCaptured;
                        }
                    }
                }
    ...
    case BISHOP_TO:
        ...
        for (int dx = -2; dx <= 2; dx += 4) {
            for (int dy = -2; dy <= 2; dy += 4) {
                sqDst = sqSrc + dx + dy * 16;
                sqPin = sqSrc + dx/2 + (dy/2) * 16;
                if (IN_BOARD(sqDst) && BISHOP_SPAN(sqSrc, sqDst) &&
                    ucpcSquares[sqPin] == 0) {
                    pcCaptured = ucpcSquares[sqDst];
                      #ifdef JIEQIMODE
                        pcCaptured = CHECK_DARK_PIECE(pcCaptured) ? pcCaptured - 32: pcCaptured;
