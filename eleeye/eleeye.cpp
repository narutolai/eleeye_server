...
int main(void) {
  ...
  while (!Search.bQuit) {
    ...
    case UCCI_COMM_SETOPTION:
      ...
      break;
    case UCCI_COMM_POSITIONGO: //新增的 position+go 指令的合并
      BuildPos(Search.pos, UcciComm);
      Search.pos.nDistance = 0;
      Search.pos.PreEvaluate();
      Search.nBanMoves = 0; //这里不要break 直接走下面的UCCI_COMM_GO

      Search.bPonder = UcciComm.pbPonder;
      Search.bDraw = UcciComm.pbDraw;
      switch (UcciComm.pGo) {
        case UCCI_GO_DEPTH:
          Search.nGoMode = GO_MODE_INFINITY;
          Search.nNodes = 0;
          SearchMain(UcciComm.pnDepth);
          break;
        case UCCI_GO_NODES:
          Search.nGoMode = GO_MODE_NODES;
          Search.nNodes = UcciComm.pnNodes;
          SearchMain(UCCI_MAX_DEPTH);
          break;
        case UCCI_GO_TIME_MOVESTOGO:
        case UCCI_GO_TIME_INCREMENT:
          Search.nGoMode = GO_MODE_TIMER;
          if (UcciComm.pGo == UCCI_GO_TIME_MOVESTOGO) {
            // 对于时段制，把剩余时间平均分配到每一步，作为适当时限。
            // 剩余步数从1到5，最大时限依次是剩余时间的100%、90%、80%、70%和60%，5以上都是50%
            Search.nProperTimer = UcciComm.pnTime / UcciComm.pnMovesToGo;
            Search.nMaxTimer = UcciComm.pnTime * MAX(5, 11 - UcciComm.pnMovesToGo) / 10;
          } else {
            // 对于加时制，假设棋局会在20回合内结束，算出平均每一步的适当时限，最大时限是剩余时间的一半
            Search.nProperTimer = UcciComm.pnTime / 20 + UcciComm.pnIncrement;
            Search.nMaxTimer = UcciComm.pnTime / 2;
          }
