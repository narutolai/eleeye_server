...
static bool ParsePos(UcciCommStruct &UcciComm, char *lp) {
  ...
  else if (StrEqv(lp, "jqstartpos"))
  {
    UcciComm.szFenStr = "xxxxkxxxx/9/1x5x1/x1x1x1x1x/9/9/X1X1X1X1X/1X5X1/9/XXXXKXXXX w";
  }
  else {
    return false;
  }
...
UcciCommEnum IdleLine(UcciCommStruct &UcciComm, bool bDebug) {
  ...
  // 3b. "positiongo {fen <fen_str> | startpos} [moves <move_list>] {nodes|depth|time ...}" 指令（position + go 合一）
  } else if (StrEqvSkip(lp, "positiongo ")) {
    char *goPart = NULL;
    char *p;
    if ((p = strstr(lp, " nodes ")) != NULL && (goPart == NULL || p < goPart))
      goPart = p + 1;
    if ((p = strstr(lp, " depth ")) != NULL && (goPart == NULL || p < goPart))
      goPart = p + 1;
    if ((p = strstr(lp, " time ")) != NULL && (goPart == NULL || p < goPart))
      goPart = p + 1;
    if (goPart == NULL)
      return UCCI_COMM_UNKNOWN;
    *(goPart - 1) = '\0';
    if (!ParsePos(UcciComm, lp))
      return UCCI_COMM_UNKNOWN;
    lp = goPart;
    ...
    if (false) {
      ...
      UcciComm.pGo = UCCI_GO_DEPTH;
      UcciComm.pnDepth = Str2Digit(lp, 0, UCCI_MAX_DEPTH);
    } else if (StrEqvSkip(lp, "nodes ")) {
      UcciComm.pGo = UCCI_GO_NODES;
      UcciComm.pnNodes = Str2Digit(lp, 0, 2000000000);
    } else if (StrEqvSkip(lp, "time ")) {
      UcciComm.pnTime = Str2Digit(lp, 0, 2000000000);
      bGoTime = true;
    } else {
      UcciComm.pGo = UCCI_GO_DEPTH;
      UcciComm.pnDepth = UCCI_MAX_DEPTH;
    }
    if (bGoTime) {
      if (false) {
      } else if (StrScanSkip(lp, " movestogo ")) {
        UcciComm.pGo = UCCI_GO_TIME_MOVESTOGO;
        UcciComm.pnMovesToGo = Str2Digit(lp, 1, 999);
      } else if (StrScanSkip(lp, " increment ")) {
        UcciComm.pGo = UCCI_GO_TIME_INCREMENT;
        UcciComm.pnIncrement = Str2Digit(lp, 0, 999999);
      } else {
        UcciComm.pGo = UCCI_GO_TIME_MOVESTOGO;
        UcciComm.pnMovesToGo = 1;
      }
    }
    return UCCI_COMM_POSITIONGO;
  ...
  else if (StrEqv(lp, "ucci"))
  {
    return UCCI_COMM_UCCIOK;
  }
   else {
    return UCCI_COMM_UNKNOWN;
  }
